...
?>
    ...
    ?>
             ...
             <select name="jkn" style="width:500px;font-size:15px;">
             <!-- <option>Pilih jkn</option>  -->
             <option selected disabled> --Pilih Jabatan Kesihatan Negeri--</option>
             <?php $results = mysqli_query($conn, "SELECT * FROM data_jkn ORDER BY jkn ASC"); ?>
...
<?php  
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( !empty($gred) && !empty($bil_waran) && (-1<=($bil_isi)) && (($bil_waran) >= ($bil_isi)) ){
                ...
                require_once "../connection.php";
                    $jkn=$_POST['jkn'];
                    $sql = "INSERT INTO jkn ( id_jkn, gred, bil_waran, bil_isi, bil_kosong ) VALUES( '$jkn' , '$gred' , '$bil_waran', '$bil_isi' , '$bil_kosong')  ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                     $gred = $bil_waran = $bil_isi = $bil_kosong = $jkn = "";
                        echo "<script>
                        $(document).ready( function(){
                            $('#showModal').modal('show');
                            $('#modalHead').hide();
                            $('#linkBtn').attr('href', 'manage-jkn.php');
                    ...
                    <div class="form-input-content">
                        <div class="card login-form mb-0">
                            <div class="card-body pt-4 shadow">                       
                                    <h4 class="text-center">Tambah Bilangan Waran Jabatan Kesihatan Negeri</h4>
