...
?>
    ...
    ?>
             ...
             <select name="kesihatan" style="width:500px;font-size:15px;">
             <!-- <option>Pilih kesihatan</option>  -->
             <option selected disabled> --Pilih Klinik Kesihatan Kerajaan--</option>
             <?php $results = mysqli_query($conn, "SELECT * FROM data_kesihatan ORDER BY kesihatan ASC"); ?>
             <?php while ($row = mysqli_fetch_array($results)) { ?> 
             <option value="<?php echo $row ['id'];?>" >
             - <?php echo $row['kesihatan']; ?>
             </option>
             <?php } ?>
             </select>
             </div>
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( !empty($gred) && !empty($bil_waran) && (-1<=($bil_isi)) && (($bil_waran) >= ($bil_isi)) ){
                ...
                require_once "../connection.php";
                    ...
                    $sql = "INSERT INTO kesihatan ( id_kesihatan, gred, bil_waran, bil_isi, bil_kosong ) VALUES( '$kesihatan' , '$gred' , '$bil_waran', '$bil_isi' , '$bil_kosong')  ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                     $gred = $bil_waran = $bil_isi = $bil_kosong = $kesihatan = "";
                        echo "<script>
                        $(document).ready( function(){
                            $('#showModal').modal('show');
                            $('#modalHead').hide();
                            $('#linkBtn').attr('href', 'manage-kesihatan.php');
                    ...
                    <div class="form-input-content">
                        <div class="card login-form mb-0">
                            <div class="card-body pt-4 shadow">                       
                                    <h4 class="text-center">Tambah Bilangan Waran Klinik Kesihatan Kerajaan</h4>
