...
?>
    ...
    ?>
             ...
             <select name="institusiagensi" style="width:500px;font-size:15px;">
             <!-- <option>Pilih institusiagensi</option>  -->
             <option selected disabled> --Pilih Institusi & Agensi--</option>
             <?php $results = mysqli_query($conn, "SELECT * FROM data_institusiagensi"); ?>
...
<?php  
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( !empty($gred) && !empty($bil_waran) && (-1<=($bil_isi)) && (($bil_waran) >= ($bil_isi)) ){
                ...
                // database connection
                require_once "../connection.php";
                    $institusiagensi=$_POST['institusiagensi'];
                    $sql = "INSERT INTO institusiagensi ( id_institusiagensi, gred, bil_waran, bil_isi, bil_kosong ) VALUES( '$institusiagensi' , '$gred' , '$bil_waran', '$bil_isi' , '$bil_kosong')  ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                     $gred = $bil_waran = $bil_isi = $bil_kosong = $institusiagensi = "";
                        echo "<script>
                        $(document).ready( function(){
                            $('#showModal').modal('show');
                            $('#modalHead').hide();
                            $('#linkBtn').attr('href', 'manage-agensi.php');
                    ...
                    <div class="form-input-content">
                        <div class="card login-form mb-0">
                            <div class="card-body pt-4 shadow">                       
                                    <h4 class="text-center">Tambah Bilangan Waran Institusi & Agensi</h4>
