...
?>
    ...
    ?>
             ...
             <select name="pkd" style="width:500px;font-size:15px;">
             <!-- <option>Pilih pkd</option>  -->
             <option selected disabled> --Pilih Pejabat Kesihatan Daerah--</option>
             <?php $results = mysqli_query($conn, "SELECT * FROM data_pkd"); ?>
...
<?php  
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( !empty($gred) && !empty($bil_waran) && (-1<=($bil_isi)) && (($bil_waran) >= ($bil_isi)) ){
                ...
                require_once "../connection.php";
                    $pkd=$_POST['pkd'];
                    $sql = "INSERT INTO pkd ( id_pkd, gred, bil_waran, bil_isi, bil_kosong ) VALUES( '$pkd' , '$gred' , '$bil_waran', '$bil_isi' , '$bil_kosong')  ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                     $gred = $bil_waran = $bil_isi = $bil_kosong = $pkd = "";
                        echo "<script>
                        $(document).ready( function(){
                            $('#showModal').modal('show');
                            $('#modalHead').hide();
                            $('#linkBtn').attr('href', 'manage-pkd.php');
                    ...
                    <div class="form-input-content">
                        <div class="card login-form mb-0">
                            <div class="card-body pt-4 shadow">                       
                                    <h4 class="text-center">Tambah Bilangan Waran Pejabat Kesihatan Derah</h4>
