...
<?php  
        ...
        $sql = "SELECT * FROM pkd WHERE id = $id ";
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( !empty($gred) && !empty($bil_waran) && (-1<=($bil_isi)) && (($bil_waran) >= ($bil_isi)) ){
                ...
                $sql_select_query = "SELECT id FROM pkd WHERE id = '$id' ";
                $r = mysqli_query($conn , $sql_select_query);

                    $sql = "UPDATE pkd SET gred = '$gred' , bil_waran = '$bil_waran' , bil_isi ='$bil_isi' , bil_kosong='$bil_kosong' WHERE id = $_GET[id] ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                        echo "<script>
                    ...
                    <div class="form-input-content">
                        <div class="card login-form mb-0">
                            <div class="card-body pt-4 shadow">                       
                                    <h4 class="text-center">Kemaskini Bilangan Waran Pejabat Kesihatan Daerah</h4>
