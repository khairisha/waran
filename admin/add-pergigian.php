...
?>
    ...
    ?>
             ...
             <select name="pergigian" style="width:500px;font-size:15px;">
             <!-- <option>Pilih pergigian</option>  -->
             <option selected disabled> --Pilih Klinik Pergigian Kerajaan--</option>
             <?php $results = mysqli_query($conn, "SELECT * FROM data_pergigian ORDER BY pergigian ASC"); ?>
             <?php while ($row = mysqli_fetch_array($results)) { ?> 
             <option value="<?php echo $row ['id'];?>" >
             - <?php echo $row['pergigian']; ?>
             </option>
             <?php } ?>
             </select>
             </div>
...
<?php  
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( !empty($gred) && !empty($bil_waran) && (-1<=($bil_isi)) && (($bil_waran) >= ($bil_isi)) ){
                ...
                // database connection
                require_once "../connection.php";
                    $pergigian=$_POST['pergigian'];
                    $sql = "INSERT INTO pergigian ( id_pergigian, gred, bil_waran, bil_isi, bil_kosong ) VALUES( '$pergigian' , '$gred' , '$bil_waran', '$bil_isi' , '$bil_kosong')  ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                     $gred = $bil_waran = $bil_isi = $bil_kosong = $pergigian = "";
                        echo "<script>
                        $(document).ready( function(){
                            $('#showModal').modal('show');
                            $('#modalHead').hide();
                            $('#linkBtn').attr('href', 'manage-pergigian.php');
                    ...
                    <div class="form-input-content">
                        <div class="card login-form mb-0">
                            <div class="card-body pt-4 shadow">                       
                                    <h4 class="text-center">Tambah Bilangan Waran Klinik Pergigian Kerajaan</h4>
