...
?>
    ...
    <?php
      $sql="SELECT institusilatihan
      FROM data_institusilatihan
      ORDER BY institusilatihan"; 
      $results = mysqli_query($conn, $sql); 
    ?>

             <div class="input-group">
             <select name="institusilatihan" style="width:500px;font-size:15px;">
             <!-- <option>Pilih institusilatihan</option>  -->
             <option selected disabled> --Pilih Institusi Latihan--</option>
             <?php $results = mysqli_query($conn, "SELECT * FROM data_institusilatihan"); ?>
             <?php while ($row = mysqli_fetch_array($results)) { ?> 
             <option value="<?php echo $row ['id'];?>" >
             - <?php echo $row['institusilatihan']; ?>
             </option>
             <?php } ?>
             </select>
             </div>
...
<?php  
        ...
        if( $_SERVER["REQUEST_METHOD"] == "POST" ){
            ...
            if( !empty($gred) && !empty($bil_waran) && (-1<=($bil_isi)) && (($bil_waran) >= ($bil_isi)) ){
                ...
                // database connection
                require_once "../connection.php";
                    $institusilatihan=$_POST['institusilatihan'];
                    $sql = "INSERT INTO institusilatihan ( id_institusilatihan, gred, bil_waran, bil_isi, bil_kosong ) VALUES( '$institusilatihan' , '$gred' , '$bil_waran', '$bil_isi' , '$bil_kosong')  ";
                    $result = mysqli_query($conn , $sql);
                    if($result){
                     $gred = $bil_waran = $bil_isi = $bil_kosong = $institusilatihan = "";
                        echo "<script>
                    ...
                    <div class="form-input-content">
                        <div class="card login-form mb-0">
                            <div class="card-body pt-4 shadow">                       
                                    <h4 class="text-center">Tambah Bilangan Waran Institusi Latihan</h4>
